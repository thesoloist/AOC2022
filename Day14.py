...
def drop(pt, state_map, has_floor):
    my_state = []
    edge = None
    ...
    if(sand_y+1 >= len(my_state)): #gonna fall out bottom
        ...
        my_state, edge = drop((sand_x, sand_y+1), my_state, has_floor)
    else:
        can_left = (sand_x!=0)
        if(can_left):
            can_left = my_state[sand_y+1][sand_x-1] == '.'
        elif(has_floor):
            # if we are at the edge AND on the floor, add it and return right here
            if(sand_y == len(my_state)-2):
                my_state[sand_y][sand_x] = 'o'
                return (my_state, None)                    
            else:
                return (None, True)
        ...
        if can_left:            
            my_state, edge = drop((sand_x-1, sand_y+1), my_state, has_floor)
        elif can_right:
            my_state, edge = drop((sand_x+1, sand_y+1), my_state, has_floor)
        else:            
            my_state[sand_y][sand_x] = 'o'
...
def solve(drop_point, floor_expand):
    ...
    while True:
        ...
        (cur_state, edge) = drop(mdp, updated_map, floor_expand>0)
        if(cur_state == None):
